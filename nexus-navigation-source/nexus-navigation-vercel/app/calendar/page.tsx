...
import {
  parseAIJson,
  validateCalendarDraftSuggestion,
...
export default function CalendarPage() {
  ...
  function saveEvent(event: FormEvent) { event.preventDefault(); if (!form.title.trim()) return; if (editingId) { setEvents((items) => items.map((item) => item.id === editingId ? updateCurrentEvent(item, form, { createId: () => crypto.randomUUID(), resourceIds: resources.map((resource) => resource.id) }) : item)); } else setEvents((items) => [...items, ...buildEvents(form)]); setEditorOpen(false); setEditingId(null); setForm({ ...emptyForm, resources: [], date: dateKey(anchor) }); setResourceSuggestion(null); setResourceSuggestionMessage(""); }
  ...
  async function askAi(event: FormEvent) {
    ...
    try {
      ...
      const proposal = validateCalendarDraftSuggestion(parseAIJson(text), resources, emptyForm, aiPermissions.planning);
