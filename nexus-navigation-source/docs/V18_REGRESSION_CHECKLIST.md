...
- [ ] Hide the homepage extension entry, refresh, and verify it remains hidden.
- [ ] Restore the extension entry from Resource management.
...
- [ ] Download Edge, Chrome and Safari ZIP files from the homepage entry and open the Markdown installation guide.
- [ ] Confirm a fresh/reset default data set no longer contains XMRth, while migration preserves an existing user-created XMRth Resource.
