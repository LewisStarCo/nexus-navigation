...
当前 Nexus 首页已经提供可隐藏的 **获取扩展** 入口。它会根据浏览器在本地选择仓库随网站发布的 ZIP，并链接到 [`EXTENSION_INSTALLATION.md`](EXTENSION_INSTALLATION.md)。这改善了下载体验，但不会绕过浏览器的安装确认。
...
当前采用的体验是：右上角提供安静的小入口，不主动弹窗；用户可隐藏，并可从设置中恢复。商店地址尚未创建前，入口直接提供本项目维护的开发者安装包。
