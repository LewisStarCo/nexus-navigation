...
async function openOrFocusNexus() {
  ...
  if (typeof tab.windowId === "number" && chrome.windows?.update) await chrome.windows.update(tab.windowId, { focused: true });
  return tab;
}
...
async function queueCapture(payload) {
  ...
  const normalizedUrl = new URL(String(payload.url)).toString();
  const existingIndex = queuedCaptures.findIndex((item) => {
    try { return new URL(String(item?.url || "")).toString() === normalizedUrl; } catch { return false; }
  });
  const capture = {
    id: existingIndex >= 0 ? queuedCaptures[existingIndex].id : crypto.randomUUID(),
    title: String(payload.title || "").trim(),
    description: String(payload.description || "").replace(/\s+/g, " ").trim().slice(0, 240),
    url: normalizedUrl,
    category: String(payload.category || "__nexus_unclassified__"),
    createdAt: Date.now()
  };
  if (existingIndex >= 0) queuedCaptures[existingIndex] = capture;
  else queuedCaptures.push(capture);
