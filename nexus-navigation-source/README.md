...
首页右上角现在提供可隐藏的 **获取扩展** 小入口。它会在本地识别 Edge、Chrome 或 Safari，并直接下载仓库中对应的 ZIP 扩展包；弹窗也提供其他浏览器版本和独立安装说明。隐藏后可从 **管理资源 → 浏览器扩展入口** 恢复。详细步骤见 [`docs/EXTENSION_INSTALLATION.md`](docs/EXTENSION_INSTALLATION.md)。
