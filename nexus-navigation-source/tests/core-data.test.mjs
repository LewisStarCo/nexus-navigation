...
test("default data is a complete current NexusData value", () => {
  ...
  assert.equal(data.settings.extensionEntryHidden, false);
  assert.ok(data.categories.length > 0);
  assert.ok(data.resources.every((resource) => resource.type === "website"));
  assert.equal(data.resources.some((resource) => resource.type === "website" && resource.url.includes("xmrth1.net")), false);
