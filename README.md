...
- **Used in Category**：允许用户主动请求 AI，根据网页标题、地址、说明和现有分类推荐一个分类。
- **Do not use AI in any situation**：同时关闭上述两项，不发起任何 AI 请求。

关闭 AI 权限不会删除 Provider 或 API Key。Calendar 建议仍需用户确认后才会加入日程；分类建议仍需用户点击“完成”后才会保存。
...
本地保存的数据包括导航内容、分类顺序、未归类网页、临时网页、用户名、时区、主题、搜索引擎、Event、重复日程、用户主动保存的 BYOK 设置和 AI 使用权限。
