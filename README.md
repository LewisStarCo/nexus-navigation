...
当前版本支持从主流日历应用导出的文件中一次性导入日程，不需要登录或 OAuth。它不会持续同步外部账户；Google、Microsoft 和 Apple 的账户级自动同步仍需要未来配置 OAuth 开发者凭据。

### 从外部应用导入

1. 在 Apple Calendar、Google Calendar 或 Outlook 中导出/下载 `.ics` 文件。
2. Microsoft To Do 可以使用 Microsoft 账户数据导出中的 `.csv` 或 `.json` 任务文件；兼容的 `.ics` 也可以导入。
3. 打开 Nexus Calendar，点击 `Import`。
4. 选择来源并上传文件。
5. 检查预览，确认后点击导入。

Nexus 会尝试识别标题、日期、时间、分类、完成状态和常见重复规则。不同平台或语言版本的 CSV 字段可能不同；无法识别的字段会使用安全默认值，原文件不会被修改。
