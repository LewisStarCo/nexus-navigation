...
```ts
{
  ...
  source: "local | calendar | google-calendar | outlook-calendar | apple-calendar | ai-suggestion"
}
```

当前版本尚未接入 Google Calendar、Outlook Calendar 或 Apple Calendar，也未实现 OAuth；相关架构已经预留。
...
- Google Calendar、Outlook Calendar 和 Apple Calendar 集成。
- Event 系列级别的批量编辑与删除。
- 可选的 Vercel AI 请求中转层。
